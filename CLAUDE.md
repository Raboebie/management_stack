...
- **Traefik reverse proxy + TLS**: Traefik v3 provides HTTPS for all web UIs via `*.home` wildcard cert generated by mkcert. HTTP automatically redirects to HTTPS. Existing port mappings are kept for backward compat and Icinga2 health checks. Traefik discovers services via Docker labels (`traefik.enable=true`).
...
- To add a new service: add to `templates/docker-compose.yml.j2` (with Traefik labels for HTTPS routing), add any needed variables to `group_vars/all.yml` (re-encrypt), add config templates, update the task list in `roles/monitoring_stack/tasks/main.yml`, and add a DNS record to `pihole_dns_records`.
